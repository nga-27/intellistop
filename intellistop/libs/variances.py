...
def calculate_time_series_variances(data_set: list,
    ...
    if mode == 'var':
        first_var = float(np.var(data_set[0:window]))
        for i in range(window):
            ts_var[i] = first_var
        for i in range(window, len(data_set)):
            ts_var[i] = float(np.var(data_set[i-window+1:i]))

    elif mode == 'std':
        first_var = float(np.std(data_set[0:window]))
        for i in range(window):
            ts_var[i] = first_var
        for i in range(window, len(data_set)):
            ts_var[i] = float(np.std(data_set[i-window+1:i]))
